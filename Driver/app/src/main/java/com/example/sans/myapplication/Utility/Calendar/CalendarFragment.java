    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        client.post("order/myRecords", params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    ...
                    for (int i = 0; i < order_array.length(); i++) {
                        items[i] = order_array.get(i).toString();
                    }
                } catch (JSONException e) {
                    Toast.makeText(CalendarFragment.this.getActivity(), "" + e, Toast.LENGTH_SHORT).show();
                }
                mAdapter.notifyDataSetChanged();
                mAdapter = new CalendarAdapter(items);
            ...
            public void onFailure(int statusCode, Header[] headers, Throwable error, JSONObject response) {
                Toast.makeText(CalendarFragment.this.getActivity(), "" + statusCode + " " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
